...
%c) Valores deseados h(0.5, -0.4, 0.0)  
    q1d = sin(0.1*t);   % Posición deseadas de la articular del eslabón 1  
    q2d = cos(0.1*t);   % Posición deseadas de la articular del eslabón 2
    
    q1dp = 0.1*cos(0.1*t);   % Posición deseadas de la articular del eslabón 1  
    q2dp =-0.1*sin(0.1*t);   % Posición deseadas de la articular del eslabón 2
    
    q1dpp =-0.1*0.1*sin(0.1*t);   % Posición deseadas de la articular del eslabón 1  
    q2dpp =-0.1*0.1*cos(0.1*t);   % Posición deseadas de la articular del eslabón 2
    
    hzd = 0.2;      % Posición deseadas del extremo operativo en el eje Z
...
%4)Modelo encontrado del roobot
   em = 0.1*rand; % Errores en el modelo Matriz de Inercia M
   ec = 0.1*rand; % Errores en el modelo Matriz de Fuerzas C
   ef = 0.1*rand; % Errores en el modelo Vector f
   
   qp = [qp1(k) qp2(k)]';
   M = 1*[1.7277+0.1908*cos(q2(k)) 0.0918+0.0954*cos(q2(k));...
           0.0918+0.0954*cos(q2(k)) 0.9184];
   C = 1*[31.8192-0.0954*sin(q2(k))*qp2(k) -0.0954*sin(q2(k))*(qp1(k)+qp2(k));...
           0.3418*sin(q2(k))*qp1(k) 12.5783];
   f = 1*[1.0256*sign(qp1(k));...
           1.7842*sign(qp2(k))];
      
%4)Ley de Control
   %a)Ley de Control para q1 y q2
    qdpp = [q1dpp(k) q2dpp(k)]'; 
    a = qdpp+K*tanh(2*qe) + D*tanh(2*qpe); 
    T_ref = M*a+C*qp+f;
...
%a)  Configuración de la animación del robot BOSCH-SR800 
 ...
 %b) Animación de movimiento del ROBOT BOSCH-SR800
     ...
     H  = plot3(hx(1),hy(1),hz(1),'b','LineWidth',3);hold on
     Hd  = plot3(hxd(1),hyd(1),hzd(1),'g','LineWidth',4);
...
for n = 1:50:length(t)
    ...
    H  = plot3(hx(1:n),hy(1:n),hz(1:n),'--b','LineWidth',2);hold on
    Hd = plot3(hxd(1:n),hyd(1:n),hzd(1:n),'g','LineWidth',4);
    pause(To)
end
